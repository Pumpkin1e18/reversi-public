...
// boardの表示
void ReversiBoard::print_board(int isInfo){
  printf(" ");
  rep(i,n)printf("%2lld", i+1);
  printf("\n");
  ...
  rep(i,n){
    printf("%lld|", i+1);
    rep(j,n){
      if(black[i*n+j])printf("%2s", "〇");
      else if(white[i*n+j])printf("%2s", "●");
      else if(can_put(i, j, BLACK))printf("%2s", "*");
      else printf("%2s", "・");
    }
    printf("|\n");
  }
...
// Bitの二次元表示
void ReversiBoard::print_bit(const Bit &bit){
  rep(i,n){
    rep(j,n)printf("%d", (int)bit[i*n+j]);
    printf("\n");
  }
}

// 初期化
ReversiBoard::ReversiBoard(){init(8LL);}
...
Bit ReversiBoard::get_mask(int y, int x, int color){
  ...
  if(2LL <= x and 2LL <= y and ene[(y-1)*n+(x-1)]){   //左上
    int m = min(x, y);
  ...
  if(2LL <= x and y < n-2 and ene[(y+1)*n+(x-1)]){   //左下
    int m = min(x, n-y-1);
  ...
  if(x < n-2 and 2LL <= y and ene[(y-1)*n+(x+1)]){   //右上
    int m = min(n-x-1, y);
...
int ReversiBoard::can_put(int y, int x, int color){
  if(y < 0LL or y >= n or x < 0LL or x >= n or !can[y*n+x])return 0;
...
Bit ReversiBoard::get_rev(Bit black_s, Bit white_s){
  ...
  if(v[0] != EMPTY){  // 左からつながりを見る
    rep(i,n){if(color != v[i])break;rev.set(i);}
  }
  color = v[n-1];
...
P ReversiBoard::get_establish(int color){
  ...
  rep(i,4){ // 4辺の確定石を見る
    ...
    rep(j,n)if(res[j])rev.set(u[i][j]);
  }
